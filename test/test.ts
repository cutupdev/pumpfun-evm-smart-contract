...
describe("Test Pump Fun", function () {
    ...
    describe("Token Create", function () {
        ...
        it("Token Factory Deployment", async function () {
            ...
            hardhatPumpFun = await hre.ethers.deployContract("PumpFun", [config.feeRecipient, config.feeAmount, config.feeBasisPoint]);

            console.log("Deployed TokenFactory Address :: ", await hardhatTokenFactory.getAddress());
            console.log("Deployed PumpFun Contract Address :: ", await hardhatPumpFun.getAddress());
        ...
        it("Init Token Factory Variables", async function () {
            ...
            expect(pumpFunAddress).to.equal(await hardhatTokenFactory.contractAddress());

        });
        it("Token Creating on Token Factory Contract", async function () {
            await hardhatTokenFactory.deployERC20Token(
                config.name,
                config.symbol,
                {
            ...
            const tokenBondingCurve = await hardhatPumpFun.getBondingCurve(testToken[0]);
    ...
    describe("Buy/Sell Function Check", function () {
        it("Buy Function", async function () {
            ...
            Erc20Token = await hre.ethers.getContractAt(
                "IERC20",
                testToken[0]
            );
            ...
            const tokenReceivedWithLiquidity = exchangeRate(amount, tokenBondingCurve);
            ...
            const buyTx = await hardhatPumpFun.connect(addr1).buy(testToken[0], tokenReceivedWithLiquidity, maxEthAmount, { value: ethAmount });
            const receipt = await buyTx.wait();

            console.log("------------------Fee Wallet ETH Balance Change Show--------------------------")

            const after = await hre.ethers.provider.getBalance(config.feeRecipient);
            console.log("Before Buy: ", before, "     After Buy: ", after, "\n >>>====== Change Balance ", after - before, "\n >>>====== Buy ETH Balance", 10 ** 18, "\n>>>>>>========= Percentage: ", Number((after - before) * 100n / ethAmount));
            console.log("------------------------------------------------------------------------------\n")
        ...
        it("Sell Function", async function () {
            ...
            await Erc20Token.connect(addr1).approve(await hardhatPumpFun.getAddress(), amount);
            const sellTx = await hardhatPumpFun.connect(addr1).sell(testToken[0], amount, minEthAmount);
